// Billing + metering for the hosted DecisionMatrix MCP server.
// - API keys + daily usage counters live in Cloudflare KV (env.DECISIONMATRIX_KV).
// - Payments via Stripe Checkout (subscription). Keys are provisioned on the
//   success page and revoked/restored by the subscription webhook.
// Never logs or returns the Stripe secret.
//
// This mirrors PrecisionCalc's billing model 1:1. DecisionMatrix has no
// "paid-only" tools — every tool is available on every tier; paid plans simply
// raise the daily call quota. The single gating point is `consumeQuota`.
...
export function planLimits(env) {
  return {
    free: parseInt(env.FREE_DAILY || "15", 10),
...
export async function identify(request, env) {
  const kv = env.DECISIONMATRIX_KV;
...
export function upsell(env, kind, ctx = {}) {
  const base = env.DECISIONMATRIX_BASE_URL || "https://decisionmatrix-mcp.pages.dev";
...
export async function createCheckout(env, plan) {
  ...
  const base = env.DECISIONMATRIX_BASE_URL;
  const session = await stripe(env, "POST", "/checkout/sessions", {
    ...
    "metadata[app]": "decisionmatrix",
    "metadata[plan]": plan,
    "subscription_data[metadata][app]": "decisionmatrix",
...
export async function createPortal(env, apiKey) {
  ...
  const session = await stripe(env, "POST", "/billing_portal/sessions", {
    ...
    return_url: `${env.DECISIONMATRIX_BASE_URL}/#pricing`,
  });
  return session.url;
}
